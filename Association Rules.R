...
rules3 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.8), appearance = list (default="lhs",rhs="whole milk"), control = list (verbose=F))#verbose removes the unnessesary output text
inspect(rules3[1:5])
inspect(rules3)
...
inspect(subset2) #both the items should be present on the LHS
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') )
inspect(subset2a) #either of the item should be there 
